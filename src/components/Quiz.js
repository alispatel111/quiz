...
import './Quiz.css'; 
import { expirePassword } from '../authService'; // Import to set password expired

function Quiz({ onLogout, email }) { // Accept email as a prop
    ...
    const [isPasswordExpired, setIsPasswordExpired] = useState(false); // Track password expiry
    ...
    const changeQuestion = () => {
        ...
        if (currentQuestion < questions.length - 1) {
            ...
            setShowResult(true);
            setIsPasswordExpired(true); // Set the password as expired
            expirePassword(email); // Use the email passed as prop
        }
    };
    ...
    const resetAll = () => {
        ...
        setPassword('');
        setIsPasswordVerified(false);
        setIsPasswordExpired(false); // Reset password expiry status
    };
            ...
            <div className="question-container">
                {showResult ? (
                    <QuizResult 
                        ...
                        isPasswordExpired={isPasswordExpired} // Pass the expiry status here
                    />
                ) : (
                    <>
